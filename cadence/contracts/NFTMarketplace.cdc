import NonFungibleToken from "./NonFungibleToken.cdc"
import NPM from "./NPM"
import FungibleToken from "./FungibleToken"
import FlowToken from "./FlowToken"

pub contract NFTMarketplace {
    pub struct SaleItem {
        pub let price: UFix64
        pub let nftRef: &NPM.NFT
        
        init(price: UFix64, nftRef: &NPM.NFT) {
            self.price = price
            self.nftRef = nftRef
        }
    }

    pub resource interface SaleCollectionPublic {
        pub fun purchaseNFT(id: UInt64, creator: Address, creatorRoyalty: UInt64, recipientCollection: &NPM.Collection{NonFungibleToken.CollectionPublic}, payment: @FlowToken.Vault)
        pub fun getPrice(id: UInt64): UFix64
        pub fun getIDs(): [UInt64]
    }

    // The Vault of the Marketplace where it will receive the cuts on each sale
    pub let marketplaceWallet: Capability<&FlowToken.Vault{FungibleToken.Receiver}>

    // Event that is emitted when a sale item is listed
    pub event SaleItemListed(id: UInt64, price: UFix64)

    // Event that is emitted when a sale item is removed
    pub event SaleItemRemoved(id: UInt64)
    
    // Event that is emitted when a sale item is purchased
    pub event SaleItemPurchased(id: UInt64, price: UFix64, seller: Address?)

    pub resource SaleCollection: SaleCollectionPublic {
        pub var prices: {UInt64: UFix64}
        pub let OwnerNFTCollection: Capability<&NPM.Collection>
        pub let FlowTokenVault: Capability<&FlowToken.Vault{FungibleToken.Receiver}>

        pub fun listNFT(id: UInt64, price: UFix64) {
            pre {
                price >= 0.0: "Price cannot be negative"
                self.OwnerNFTCollection.borrow()!.getIDs().contains(id): "Does not own this NFT"
            }
            self.prices[id] = price
            emit SaleItemListed(id: id, price: price)
        }

        pub fun unlistNFT(id: UInt64) {
            self.prices.remove(key: id)
            emit SaleItemRemoved(id: id)
        }

        pub fun purchaseNFT(id: UInt64, creator: Address, creatorRoyalty: UInt64, recipientCollection: &NPM.Collection{NonFungibleToken.CollectionPublic}, payment: @FlowToken.Vault) {
            let price = self.prices[id]
            pre {
                payment.balance == price: "Payment is not equal to the price of NFT"
            }

            if (creatorRoyalty == nil) {
                creatorRoyalty = NPM.getRoyaltyCut();
            } 

            let creatorAccount = getAccount(creator);
            let creatorWallet = creatorAccount.getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver).borrow()!
            let creatorAmount = price * creatorRoyalty / 100;
            let tempCreatorWallet <- payment.withdraw(amount: creatorAmount)
            creatorWallet.deposit(from: <-tempCreatorWallet)

            let marketplaceWallet = NFTMarketplace.marketplaceWallet.borrow()! 
            let marketAmount = price * NPM.getMarketplaceCut();
            let tempMarketWallet <- payment.withdraw(amount: marketAmount)
            marketplaceWallet.deposit(from: <-tempMarketWallet)

            recipientCollection.deposit(token: <- self.OwnerNFTCollection.borrow()!.withdraw(withdrawID: id))
            self.prices.remove(key: id)
            self.FlowTokenVault.borrow()!.deposit(from: <- payment)
            emit SaleItemPurchased(id: id, price: price, seller: self.owner?.address)
        }

        pub fun getPrice(id: UInt64): UFix64 {
            return self.prices[id]!
        }

        pub fun getIDs(): [UInt64] {
            return self.prices.keys
        }

        init(OwnerNFTCollection: Capability<&NPM.Collection>, FlowTokenVault: Capability<&FlowToken.Vault{FungibleToken.Receiver}>) {
            self.prices = {}
            self.OwnerNFTCollection = OwnerNFTCollection
            self.FlowTokenVault = FlowTokenVault
        }
    }

    pub fun createSaleCollection(OwnerNFTCollection: Capability<&NPM.Collection>, FlowTokenVault: Capability<&FlowToken.Vault{FungibleToken.Receiver}>): @SaleCollection {
        return <- create SaleCollection(OwnerNFTCollection: OwnerNFTCollection, FlowTokenVault: FlowTokenVault)
    }

    init() {
        self.marketplaceWallet = self.account.getCapability<&FlowToken.Vault{FungibleToken.Receiver}>(/public/flowTokenReceiver)
    }
}